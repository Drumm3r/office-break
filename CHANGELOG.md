...
- **Active break exercise changed on app resume** — When the user backgrounded the app mid-break and reopened it (or the Activity was recreated via rotation or process death), `TimerScreen`'s `LaunchedEffect(timerState)` re-fired against the still-`Expired` state, triggering `TimerViewModel.onTimerExpired()` a second time. The repick overwrote the exercise that had just been restored from `SavedStateHandle`, so the dialog showed a different exercise than before. `onTimerExpired()` is now idempotent — early-returns when `_currentExercise` is already set — so the same break always displays the same exercise until the user taps Done.
